...
class Model21Risk(BaseModel):
    ...
    def _flag(value) -> bool:
        if isinstance(value, bool):
            return value
        return str(value).strip().lower() in {"是", "y", "yes", "true", "1", "严禁投标", "red"}

    @staticmethod
    def _qcc_desc(row, default: str) -> str:
        details = str(row.get("qcc_risk_details", "") or "").strip()
        matched_name = str(row.get("qcc_company_name", "") or "").strip()
        parts = [default]
        if matched_name:
            parts.append(f"企查查匹配企业：{matched_name}")
        if details:
            parts.append(f"风险详情：{details[:500]}")
        return "；".join(parts)
    ...
    def run(self, dmp, appendices, region_auth=None):
        ...
        for idx, row in df.iterrows():
            ...
            # ==================================================================
            if self._flag(row.get("qcc_is_shixin", False)):
                violations.append({
                    "rule": "失信被执行人业主",
                    "desc": self._qcc_desc(
                        row,
                        f"客户{customer}被企查查标记为失信被执行人，触碰模型2.1严禁投标底线",
                    ),
                    "severity": "严禁投标"
                })

            if self._flag(row.get("qcc_has_zhongjian_lawsuit", False)):
                status = str(row.get("qcc_zhongjian_status", "") or "").strip()
                suffix = f"（{status}）" if status else ""
                violations.append({
                    "rule": "与中建存在重大诉讼业主",
                    "desc": self._qcc_desc(
                        row,
                        f"客户{customer}与中建存在重大诉讼{suffix}，触碰模型2.1严禁投标底线",
                    ),
                    "severity": "严禁投标"
                })

            if self._flag(row.get("qcc_is_weiyue", False)):
                violations.append({
                    "rule": "已违约或破产重整客户",
                    "desc": self._qcc_desc(
                        row,
                        f"客户{customer}存在已违约、被执行、终本、限高、股权冻结或破产重整等风险，触碰模型2.1严禁投标底线",
                    ),
                    "severity": "严禁投标"
                })
