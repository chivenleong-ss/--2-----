...
from data_loader.data_adapter import build_unified_projects, enrich_region_data, load_bid_report
...
def main():
    ...
    if not args.report_only:
        ...
        #
        # 判定规则：
        #   满足 ② AND ③（180天无产值 且 360天无收款）→ 定义为"未开工" → 剔除
        #   例外：项目状态为"停工/退场/停缓建" → 不判定为未开工（属正常施工后终止，保留分析）
        #
        # 输出：
        #   ① 未开工项目清单（剔除，不纳入下游模型） + 各项目具体原因
        #   ② 停工/退场/停缓建项目清单（保留，但标记供模型2.2做盈利分析时重点关注）

        before_count = len(dmp)
        before_amt = dmp["签约额（元）"].apply(lambda x: x if pd.notna(x) else 0).sum()
        now = datetime.now()

        # Check if supplemental fields are available
        ...
        unstarted_reasons = []   # (proj_code, proj_name, contract_amt, days, reasons[])
        stopped_projects = []    # (proj_code, proj_name, status, contract_amt, days)

        for idx, row in dmp.iterrows():
            code = str(row.get("项目编码", ""))
            name = str(row.get("项目名称", ""))
            contract_amt = safe_float(row.get("签约额（元）", 0))
            status = str(row.get("项目状态", "")).strip() if has_status else ""

            start = row.get("开工时间")
            sign = row.get("签约时间")

            # 签约为空 → 未签约项目，保留给模型1.4检测僵尸投标
            ...
            # === 停工/退场/停缓建 → 独立标记，不剔除 ===
            if any(kw in status for kw in ["停工", "退场", "停缓建"]):
                stopped_projects.append((code, name, status, contract_amt, days_since_sign))
                continue
            ...
            if is_unstarted:
                unstarted_reasons.append((code, name, contract_amt, days_since_sign, reasons))

        # === 输出：未开工项目 ===
        excluded_count = len(unstarted_reasons)
        excluded_amt = sum(r[2] for r in unstarted_reasons)
        ...
        prefilter_summary = {
            ...
            "stopped_project_amount_yi": round(sum(r[3] for r in stopped_projects) / 1e8, 4) if stopped_projects else 0.0,
            ...
            "unstarted_projects": [
                {
                    ...
                    "contract_amount_yi": round(float(amt) / 1e8, 4),
    ...
    if args.report_only:
        ...
        # ===== Discrete Analysis (风险-收益九宫格) =====
        print("\n[4.2/5] Running discrete risk-return analysis...")
        appendix_df = appendices.get("appendix_1", pd.DataFrame())
        discrete_analyzer = DiscreteAnalyzer(config)
        discrete_results = discrete_analyzer.run(all_results, dmp, appendix_df, region_auth)
        discrete_summary = discrete_results.get("summary", {})
        print(f"  Discrete analysis: {discrete_summary.get('total_projects', 0)} projects -> 9-grid")
        ...
        # Save discrete results for web
        disc_path = os.path.join(args.output_dir, "model_outputs", "_discrete_results.pkl")
        with open(disc_path, "wb") as f:
            pickle.dump(discrete_results, f)

        # ===== Business Health Analysis (方案1：非历史追溯部分) =====
        print("\n[4.3/5] Running business health analysis...")
