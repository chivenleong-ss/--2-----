...
class Model12Business(BaseModel):
    ...
    def run(self, dmp, appendices, region_auth=None):
        ...
        struct_targets = config_15.get("业务结构目标", {})
        framework_1236 = config_15.get("1236框架", {})
        region_targets = config_15.get("区域发展目标", {})
        tolerance = struct_targets.get("偏差容忍度", 0.05)
                    ...
                    if target_pct is not None and target_pct > 0:
                        ...
                        if deviation > tolerance:
                            ...
                            findings.append({
                                ...
                                "问题描述": (
                                    f"【业务结构目标】{seg}实际占比{actual_pct:.1%}，"
        ...
        if "申报单位" in df.columns and "签约额（元）" in df.columns:
            for unit, group in df.groupby("申报单位"):
                ...
                if "业务类型" in group.columns:
                    infra_amt = group[group["业务类型"].astype(str).str.contains("基础设施", na=False)]["签约额（元）"].apply(safe_float).sum()
                    infra_pct = infra_amt / total if total > 0 else 0
                    if "基础设施" in str(unit) and infra_pct < 0.70:
                        findings.append({
                            "模型编号": "1.2",
                            "申报单位": unit,
                            "问题分类": "专业定位偏离",
