...
APPENDIX_EXCEL = str(BASE / "【725】附件：营销质量管理专项审计附表（定稿）(1).xlsx")
...
def _read_excel_sheet(sheet_pattern: str, data_start_row: int, filepath: str = None) -> pd.DataFrame:
    ...
    wb = openpyxl.load_workbook(filepath or APPENDIX_EXCEL, data_only=True)
...
def _load_merged_appendix_1(filepath: str) -> pd.DataFrame:
    ...
    if not all(col in available for col in MERGED_APPENDIX_REQUIRED_COLUMNS):
        return pd.DataFrame()
...
def load_appendix_1(filepath: str = None) -> pd.DataFrame:
    ...
    """
    filepath = filepath or APPENDIX_EXCEL
